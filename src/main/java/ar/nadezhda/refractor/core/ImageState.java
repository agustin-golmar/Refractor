...
public class ImageState {
	...
	public Image operate(final ImageState other, final DoubleBinaryOperator op) {
		if (this.image.getHeight() != other.image.getHeight() || this.image.getWidth() != other.image.getWidth()
		...
		Image res = new Image(this.image.source + "_" + other.image.source, this.image.getChannels(),
				this.image.getWidth(), this.image.getHeight());
		for (int c = 0; c < this.image.getChannels(); c++) {
			for (int w = 0; w < this.image.getWidth(); w++) {
				for (int h = 0; h < this.image.getHeight(); h++) {
					res.data[c][w][h] = op.applyAsDouble(this.image.data[c][w][h], other.image.data[c][w][h]);
	...
	public Image unaryOp(DoubleUnaryOperator op) {
		Image res = new Image(this.image.source, this.image.getChannels(), this.image.getWidth(),
	...
	public Image increaseContrast(double scalar) {
		...
		for (int c = 0; c < image.getChannels(); c++) {
			for (int w = 0; w < image.getWidth(); w++) {
				for (int h = 0; h < image.getHeight(); h++) {
					res.data[c][w][h] = 255 * (image.data[c][w][h] - (mean[c] - stdDev[c] / scalar))
							/ (2 * stdDev[c] / scalar);
					if (res.data[c][w][h] < 0) {
						res.data[c][w][h] = 0;
					}
		...
		for (int c = 0; c < image.getChannels(); c++) {
			for (int w = dimension / 2; w < image.getWidth() - dimension / 2; w++) {
				for (int h = dimension / 2; h < image.getHeight() - dimension / 2; h++) {
					...
					Arrays.sort(window);
					res.data[c][w][h] = window.length % 2 == 1 ? window[window.length / 2]
							: window[window.length / 2] / 2 + window[window.length / 2 + 1] / 2;
		...
		for (int i = 0; i < dimension; i++) {
			for (int j = 0; j < dimension; j++) {
				...
				if (-1 < ew && ew < image.getWidth() && -1 < eh && eh < image.getHeight()) {
					window[i][j] = Math.exp(
							-(((w - ew) * (w - ew)) + ((h - eh) * (h - eh))) / (2.0 * (dimension / 3) * (dimension / 3))
